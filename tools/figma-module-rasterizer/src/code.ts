type CandidateType = "FRAME" | "COMPONENT" | "INSTANCE" | "COMPONENT_SET" | "GROUP";
...
type UIMessage =
  ...
  | { type: "download"; id?: string; scale: number }
  | { type: "replace"; id?: string; scale: number; keepOriginal: boolean };
...
function collectCandidates(): Candidate[] {
  const roots: (PageNode | ExportableNode)[] = figma.currentPage.selection.length > 0
    ? figma.currentPage.selection.filter(isScanRoot)
    : [figma.currentPage];
...
function isScanRoot(node: SceneNode): node is ExportableNode {
  return isExportableNode(node) || "findAll" in node;
}

function isCandidateNode(node: BaseNode): node is ExportableNode & { type: CandidateType } {
  return (
    isExportableNode(node) &&
    CANDIDATE_TYPES.has(node.type as CandidateType) &&
    node.visible &&
    node.width > 0 &&
    node.height > 0 &&
    !node.name.startsWith("[PNG]")
  );
}

function isExportableNode(node: BaseNode): node is ExportableNode {
  return "exportAsync" in node && "visible" in node && "width" in node && "height" in node;
}

async function getTargetNode(id?: string) {
  if (id) {
...
async function getExportableNode(id: string) {
...
async function exportNode(node: ExportableNode, scale: number) {
...
async function replaceWithPng(
  node: ExportableNode,
  options: { scale: number; keepOriginal: boolean }
) {
...
function setOptionalPluginData(node: BaseNode, data: Record<string, string>) {
  for (const [key, value] of Object.entries(data)) {
    try {
      node.setPluginData(key, value);
    } catch {
      // Local development plugins may not have a manifest id yet.
    }
  }
}

function copyPositionAndLayout(source: ExportableNode, target: RectangleNode) {
  ...
  for (const prop of props) {
    if (prop in sourceAny && prop in targetAny) {
      try {
        ...
        // Some layout props are read-only or invalid for specific parent contexts.
      }
    }
  }
}

function toCandidate(node: ExportableNode & { type?: string }, depth = 0, selected = false): Candidate {
  return {
    id: node.id,
    name: node.name,
    type: node.type as CandidateType,
...
function getDepth(node: BaseNode) {
  let depth = 0;
...
function getPath(node: BaseNode) {
