...
| 组件 | 类型 | 作用 |
|------|------|------|
| **figma-module-rasterizer** | Figma 插件 | 在 Figma 里选中一个模块，一键导出为 PNG 并**原位替换**成图片图层——把要占位的模块「栅格化」。整个流程的第一步。 |
| **figma-handoff** | Claude Code 插件 / 工作流 | 把「由占位图区块拼成的 Figma 整页稿」转成标准化的纯 HTML/CSS 前端交付包（每区块一张 PNG 占位、flex 布局，不含交互/业务逻辑）。流程的第二步。 |
...
```
① Figma 里栅格化取图              ②  跑工作流出交付包
   figma-module-rasterizer   ──►    figma-handoff
   选中模块→一键导成 PNG            读整页稿→拼装→HTML 交付包
```

先用 **figma-module-rasterizer** 在 Figma 里把要占位的模块转成 PNG，得到一份「由占位图拼成的整页稿」；再用 **figma-handoff** 把这份稿子转成纯 HTML/CSS 交付包。下面按这个顺序说明。

---

## 第一步 · figma-module-rasterizer（在 Figma 里取占位图）

工作流最麻烦的一步是取占位图——MCP 不能整节点导出 PNG。这个 Figma 插件把这步做成一键操作：**选中模块 → 导出 PNG → 原位替换成图片图层**。
...
## 第二步 · figma-handoff（整页稿 → HTML 交付包）
...
这个工作流的全部输入都来自 Figma MCP——没有第 1、2 条它无法工作。
