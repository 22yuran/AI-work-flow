...
Integer CSS tokens (px, gap, padding, sizes) — **except border/divider widths, which keep Figma's actual value: a `0.5px` hairline stays `0.5px`, never rounded up**; flex auto-sizing (no absolute positioning / hardcoded content widths except page skeleton); no invented logic; no new deps; no full-app scaffolding; **when sibling elements have different Figma values (size/weight/color/spacing), reproduce each per its actual value — never blanket one style across them**; **colors MUST come from the real `get_design_context` token value — never the `#xxx` fallback inside `var(--token, #xxx)`, and if `get_design_context` can't be reached (node's file not the active Figma tab), STOP and have the user activate it — do NOT ship sampled/guessed colors** (sampling misfires, e.g. link color `#576b95` guessed as teal `#07c1a3`); **render icons at their SVG viewBox aspect ratio and verify orientation (a frame/viewBox ratio mismatch means a rotation the export dropped — never `preserveAspectRatio:none` stretch)**; per-rule `/* block <node-id> — WxH */` comments **plus an inert `data-node-id="<node-id>"` on each block** (for the geometry self-check). **Verify by (a) re-screenshotting after any change + zooming in on <24px elements, AND (b) the machine geometry self-check (WORKFLOW step 6b) — fix every position/size deviation & missing node until it passes.** Full list in WORKFLOW.md.
