...
**基准以「语义模块」为单位、细节+间距合一**：模块指任何前端都会表达的块(头/副标题/正文/区块/可折叠项/列表项…),不是光栅化副本的 frame 嵌套。每个模块除了细节,还可用 **`gapBefore: { after, value, direction }`** 直接携带"相对前一个模块的间距"(间距挂在模块上,与顶层 `gaps[]` 等效,二选一或并用)。映射优先用前端**现成的语义 class**,`data-review-id` 仅作兜底。
